...
Move a discovery item from `docs/project/ideas/D###-*.md` into `docs/project/backlog.md` as an enriched, sprint-ready row. The Definition-of-Ready gate is non-negotiable: incomplete items stay in discovery.

## Token budget (MANDATORY)

- Read only the target discovery detail file in full; use Grep for everything else.
- `Grep` `docs/project/backlog.md` for duplicates (do not full-Read it).
...
1. **Read** the discovery detail file `docs/project/ideas/D<###>-<slug>.md` (full read — small file).
...
4. **Duplicate scan** — `Grep` `docs/project/backlog.md` for similar titles / problem keywords. If a dup exists, prefer linking the discovery item to it instead of promoting.
5. **Gather metadata** with the user: `Priority`, `Complexity`, `Target Sprint`.
6. **Allocate the next backlog ID** — `Grep` `docs/project/backlog.md` for the highest `B###` and increment.
7. **Append the enriched entry** to `docs/project/backlog.md`:
   - User story + AC + affected components + discovery ref (`see D###`)
   - Priority · Complexity · Target Sprint · Source = discovery
8. **Update the discovery index** `docs/project/ideas.md` — set State = `promoted`, append `→ B###`.
9. **Update the discovery detail file** — set State = `promoted (B###)` in the header.
10. **Refresh the slim index** — `/index-refresh` for `docs/project/backlog-index.md`.
...
- Never edit `docs/project/backlog.md` from `/discover` — that is `/promote`'s job alone.
