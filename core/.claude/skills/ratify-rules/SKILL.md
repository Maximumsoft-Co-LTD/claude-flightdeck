...
- `Grep` `docs/project/sprints/*/retro.md` for `## Candidate A-rules` — do NOT
...
1. **Gather candidates.** `Grep -n '## Candidate A-rules'`
   `docs/project/sprints/*/retro.md`. For each hit, Read just that section. A
...
3. **Decide with the operator — one candidate at a time.** Show the proposed
   ...
   - **Defer** — keep watching; not enough signal yet. Leave the candidate as
     a proposal (optionally log it in `docs/project/backlog.md`).
