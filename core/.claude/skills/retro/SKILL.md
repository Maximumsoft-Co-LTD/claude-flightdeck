...
The live per-task mini-retros at `docs/project/sprints/S<N>/tasks.md` are the PRIMARY input. The full retro file aggregates + classifies + acts on them. The backlog audit at Step 8 is a **HARD GATE** — mismatch ≠ 0 blocks the retro close.
...
1. **Read the live mini-retro file** — `docs/project/sprints/S<N>/tasks.md`. If this file is empty or missing, the sprint failed the per-task retro discipline; flag it as a process loss in the full retro.
1b. **Read `docs/project/backlog.md`** — open rows are candidates for "consumed this sprint?" reconciliation in Step 8b. Note the highest existing `F####` ID so you can pick the next free one if you append.
...
5. **Create the retro file** `docs/project/sprints/S<N>/retro.md`.
...
8b. **FOLLOWUPS.md verification (HARD GATE)** — this step **verifies** the writes performed by the `sprint-retro-author` agent (Step 5 of its body). It does NOT write follow-ups itself; the agent is the writer.
   ...
   - **Block the retro close** if `Grep docs/project/backlog.md '## Open'` returns any row whose `From task / sprint` cell references this sprint AND whose `Status` is still `open`. Every sprint-touched follow-up must end the sprint in `in-progress`, `consumed-by:<task-id>`, or `wont-do (reason)` — open is process loss.
...
11. **Mark sprint Done in `docs/project/sprints/S<N>/tasks.md`** + MOVE prose to `docs/project/sprints/S<N>/retro.md` in the SAME commit (the STATUS update protocol — keep STATUS.md a single-row pointer, not a journal).
12. **Refresh slim indexes** — `/index-refresh` for `docs/project/sprints/INDEX.md` + `docs/project/backlog-index.md` + `docs/project/sprints/S<N>/tasks.md`.
