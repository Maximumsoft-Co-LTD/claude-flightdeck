...
> **Live retro:** [`../sprints/S03/tasks.md`](../sprints/S03/tasks.md) (mini-retros appended per task; full retro at sprint close)
...
|----|-------|------|------|-------|------------|----------------------------------------------------------|--------|
| URLSH-S03.01 | Click-tracking middleware + `clicks` table | feat | M | @alice | [`D007`](../../project/sprints/S03/designs/D007-click-tracking.md) | Every `GET /{code}` writes a row; hot-path latency p99 ≤ +1ms vs baseline | [x] Done |
| URLSH-S03.02 | Redis hot cache for resolver (read path) | feat | M | @bob | [`D008`](../../project/sprints/S03/designs/D008-redis-hot-cache.md) | `GET /{code}` cache-aside; hit-rate metric exposed; invalidation on shorten/delete/regenerate | [~] In Progress — branch `feat/urlsh-s03.02-redis-cache`; awaiting hexagonal-reviewer gate |
...
| URLSH-S03.05 | **docs** Typo fix — admin page title | docs | XS | @bob | [`D009`](../../project/sprints/S03/designs/D009-typo-fix-admin-page-title.md) | Page title reads "URL Shortener" everywhere | [ ] Not Started |

## Plus (carryover, not counted toward sprint completion %)

- URLSH-S03.06 — alerting threshold tuning for `urlsh_cache_hit_rate{}` (will land once URLSH-S03.02 has produced 48h of baseline data; deferred from this sprint per [`../sprints/S03/tasks.md`](../sprints/S03/tasks.md) live entry)
...
- URLSH-S03.05 is the canonical XS example for this sample — see [`D009`](../../project/sprints/S03/designs/D009-typo-fix-admin-page-title.md) for what an XS design doc actually looks like.
